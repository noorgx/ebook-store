                    ...
                    <div class="col-lg-9">
                        <div class="shop-product-fillter">
                            <div class="totall-product">
                                <p> We found <strong class="text-brand">{{$products->total()}}</strong> Books for you from <strong class="text-brand">{{$category_name}}</strong> Category</p>
                            </div>
                                    ...
                                    <div class="product-img-action-wrap">
                                        <div class="product-img product-img-zoom">
                                            <a href="{{route('product.details',['slug'=>$product->slug])}}">
                                                <img class="default-img" src="{{ asset('material/imges')}}/{{$product->image}}" alt="{{$product->name}}">
                                                <img class="hover-img" src="{{ asset('material/imges')}}/{{$product->image}}" alt="{{$product->name}}">
                                            </a>
                                        </div>
                    ...
                    <div class="col-lg-3 primary-sidebar sticky-sidebar">
                        ...
                        <div class="widget-category mb-30">
                            ...
                            <ul class="categories">
                                ...
                                <li><a href="{{route('product.category',['slug'=>$category->slug])}}"><h5>{{$category->name}}</h5></a></li>                                @endforeach
                            </ul>
                        </div>
                        <!-- Product sidebar Widget -->
                        <div class="sidebar-widget product-sidebar  mb-30 p-30 bg-grey border-radius-10">
                            <div class="widget-header position-relative mb-20 pb-10">
                                <h5 class="widget-title mb-10">New Books</h5>
                                <div class="bt-1 border-color-1"></div>
                            </div>
                            @foreach($nproducts as $nproduct)  
                            <div class="single-post clearfix">
                                <div class="image">
                                    <img src="{{ asset('material/imges')}}/{{$nproduct->image}}" alt="{{$nproduct->name}}">
                                </div>
